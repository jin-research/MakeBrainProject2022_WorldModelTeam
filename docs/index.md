...
<div class="gif-pair">
  ...
  <figure>
    <img src="figures/sim_observation_vs_model.gif" alt="シミュレータの観測とVAEによる再構成">
    <figcaption>シミュレータの観測(左)とVAEによる再構成(右)</figcaption>
  </figure>
</div>

右の映像は、観測（左）と、それをVAEが再構成した画像（右）を並べたものです。Dreamerはこの再構成を含む内部の世界の中で行動を学習します。
