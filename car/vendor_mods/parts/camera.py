...
import glob
import cv2
from datastore_for_record import steerings_record
from datastore_for_record import throttles_record
from datastore_for_record import images_record
...
class CSICamera(BaseCamera):
    ...
    def run(self):
        ...
        #cv2.imwrite('seiji.png', frame)
        return frame
    def run_threaded(self):
        ret,came = self.camera.read()
        if ret:
            #print("images_record",type(images_record))
            #print(list(came))
            came = cv2.cvtColor(came, cv2.COLOR_BGR2RGB)
            return came
